...
export class ServServicioApi {
  ...
  getServiceById(id: number | string): Observable<Service> {
    ...
    return this.http.get<Service>(`${this.servicesUrl}/getServicioAll/${id}`);
  }
  ...
  searchServices(term?: string, categoriaId?: number): Observable<Service[]> {
    let url = `${this.servicesUrl}/search?`;
